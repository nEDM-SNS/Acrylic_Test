...
namespace {
    void PrintUsage() {
        G4cerr << " Usage: " << G4endl;
        G4cerr << " acrylic_Test [-m macro ] [-u UIsession] [-t nThreads] [-r seed] "
        << G4endl;
...
int main(int argc,char** argv)
{
    ...
    runManager-> SetUserInitialization(new Acrylic_TestDetectorConstruction());
    // Physics list
    runManager-> SetUserInitialization(new NedmPhysicsList());
    // User action initialization
    runManager->SetUserInitialization(new Acrylic_TestActionInitialization());
